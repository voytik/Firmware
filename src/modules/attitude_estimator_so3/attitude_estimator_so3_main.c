 ...
 *
 * @file attitude_estimator_so3_main.c
 *
...
__EXPORT int attitude_estimator_so3_main(int argc, char *argv[]);
...
/* NOTE : How to tune the gain? First of all, stick with this default gain. And let the quad in stable place.
   Log the steady state reponse of filter. If it is too slow, increase SO3_KP.
   If you are flying from ground to high altitude in short amount of time, please increase SO3_KI which
   will compensate gyro bias which depends on temperature and vibration of your vehicle */
/*
PARAM_DEFINE_FLOAT(SO3_KP, 1.0f); //! This parameter will give you about 15 seconds convergence time.
                                       //! You can set this gain higher if you want more fast response.
                                       //! But note that higher gain will give you also higher overshoot.
PARAM_DEFINE_FLOAT(SO3_KI, 0.05f); //! This gain will incorporate slow time-varying bias (e.g., temperature change)
...
#define ATT_ROLL_OFFS 0.0f;
#define ATT_PITCH_OFFS 0.0f;
#define ATT_YAW_OFFS 0.0f;
/**
...
usage(const char *reason)
{
	...
	fprintf(stderr, "usage: attitude_estimator_so3 {start|stop|status} [-d <devicename>] [-b <baud rate>]\n"
		"-d and -b options are for separate visualization with raw data (quaternion packet) transfer\n"
		"ex) attitude_estimator_so3 start -d /dev/ttyS1 -b 115200\n");
	exit(1);
}
...
int attitude_estimator_so3_main(int argc, char *argv[])
{
	...
	if (!strcmp(argv[1], "start")) {
		...
		if (thread_running) {
			printf("attitude_estimator_so3 already running\n");
...
int attitude_estimator_so3_thread_main(int argc, char *argv[])
{
	const unsigned int loop_interval_alarm = 6500;	// loop interval in microseconds
		...
		struct attitude_estimator_so3_params so3_params;
		struct attitude_estimator_so3_param_handles so3_param_handles;

		/* initialize parameter handles */
		parameters_init(&so3_param_handles);
		...
		perf_counter_t so3_loop_perf = perf_alloc(PC_ELAPSED, "attitude_estimator_so3");

		printf("[SO3] Subsribed to data\n");
		...
		while (!thread_should_exit) {
			...
			if (ret < 0) {
				...
				if (!control_mode.flag_system_hil_enabled) {
					fprintf(stderr,
						"[att so3] WARNING: Not getting sensors - sensor app running?\n");
				}

			} else {
				printf("[SO3] update data\n");
				...
				if (fds[1].revents & POLLIN) {
					...
					/* update parameters */
					parameters_update(&so3_param_handles, &so3_params);
				}
				...
				if (fds[0].revents & POLLIN) {
					...
					if (!initialized) {
						...
						if (!const_initialized) {
							printf("[SO3] not initialized\n");
							continue;
						}

						uint64_t timing_start = hrt_absolute_time();

						printf("[SO3] Calling the update now\n");

						// NOTE : Accelerometer is reversed.
						// Because proper mount of PX4 will give you a reversed accelerometer readings.
						NonlinearSO3AHRSupdate(gyro[0],-gyro[1],-gyro[2],-acc[0],acc[1],acc[2],mag[0],mag[1],mag[2],so3_params.Kp,so3_params.Ki, dt);
						...
						if (last_data > 0 && raw.timestamp - last_data > 12000) printf("[attitude estimator so3] sensor data missed! (%llu)\n", raw.timestamp - last_data);

						last_data = raw.timestamp;

						printf("[SO3] Publishing data\n");

						/* send out */
						att.timestamp = raw.timestamp;

						// XXX Apply the same transformation to the rotation matrix
						att.roll = euler[0] - so3_params.roll_off;
						att.pitch = euler[1] - so3_params.pitch_off;
						att.yaw = euler[2] - so3_params.yaw_off;
						...
						*/
						...
						printf("[SO3] Published data\n");

						perf_end(so3_loop_perf);
...
int parameters_init(struct attitude_estimator_so3_param_handles *h)
{
	...
	h->Kp = 	param_find("SO3_KP");
	h->Ki = 	param_find("SO3_KI");
